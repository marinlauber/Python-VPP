...
for i,tws in enumerate(np.array([3,4,5,6,7,8,10])/0.5144):
    ax.plot(dat[int(32*i):int(i*32+32),0],dat[int(32*i):int(i*32+32),1],'k',ls=stl[i],
            label=rf'${tws:.1f}$')
    ax.plot(dat[int(32*i+1),0],dat[int(32*i+1),1],'ok',lw=1,markersize=4,mfc='None')
    ax.plot(dat[int(32*i+31),0],dat[int(32*i+31),1],'ok',lw=1,markersize=4,mfc='None')
